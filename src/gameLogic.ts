...
export function evaluateTriple(c1: Cell, c2: Cell, c3: Cell): number {
...
export function generateRound(): { cells: Cell[]; target: number } {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    // (연산자, 숫자) 조합이 중복되지 않도록 생성
    // 4 operators × 9 values = 36가지 조합에서 10개를 랜덤 선택
    const allCombinations: { operator: Operator; value: number }[] = [];
    for (const op of OPERATORS) {
      for (let v = 1; v <= 9; v++) {
        allCombinations.push({ operator: op, value: v });
      }
    }
    ...
    cells = CELL_IDS.map((id, idx) => ({
      id,
      operator: shuffled[idx].operator,
      value: shuffled[idx].value,
    }));
