...
- 각 칸에는 **연산자(+, -, ×, ÷)와 숫자**가 배치됩니다 (같은 조합 없음, 중복 없음)
- 칸 3개를 선택해 타겟 넘버가 되는 수식을 만들어야 합니다
- **정답이 3개 이상 존재하는 문제만** 출제됩니다
...
### 부활 (토스앱 전용)
- 탈락 시 **전면 광고 시청**으로 탈락 면제 후 다음 라운드 진행 가능
- 광고를 보지 않으면 랭킹 등록 후 종료
...
### 랭킹
- 랭킹에는 실명 대신 **최초 로그인 시 설정한 닉네임**이 표시됩니다 (설정 후 수정 불가, 랜덤 닉네임 생성 가능)
- 게스트로 게임을 진행한 유저는 랭킹 페이지에서 로그인을 유도하며, 로그인하지 않으면 랭킹에 기록되지 않습니다
- 홈 화면에서 **랭킹 보기** 버튼으로 언제든 TOP 20 랭킹을 확인할 수 있습니다
- **토스앱** 환경에서는 `getUserKeyForGame` API로 자동 유저 식별 후, 게임 시작 전 닉네임을 설정합니다
- 같은 유저의 재등록은 **최고 점수만 유지**됩니다

## 기술 스택
- **Frontend**: React + TypeScript + Vite
- **Database**: Firebase Firestore (온라인 랭킹)
- **인앱 연동**: `@apps-in-toss/web-framework`
  - `getUserKeyForGame` — 유저 식별
  - `loadFullScreenAd` / `showFullScreenAd` — 광고 시청 부활
...
└── components/
    ├── HomeScreen.tsx         # 홈 화면 (규칙 안내 + 버튼)
    ├── PyramidBoard.tsx       # 피라미드 칸 UI
    ├── GameScreen.tsx         # 게임 진행 화면 (탈락/부활/광고 포함)
